...
Explode::Explode(bool automaticSetup):
...
Explode::Explode(const osg::Vec3& position, float scale, float intensity)
{
    setDefaults();
...
Explode::Explode(const Explode& copy, const osg::CopyOp& copyop):
    ParticleEffect(copy,copyop)
{
    if (_automaticSetup) buildEffect();
}

void Explode::setDefaults()
{
    ParticleEffect::setDefaults();

    _textureFileName = "data/fx/smoke.png";
    _emitterDuration = 1.0;
...
void Explode::setUpEmitterAndProgram()
{
